...
if [ ! -d "/home/datasim/code/datasim" ] && [ ! -d "/home/datasim/code/datasim-ui" ]
 ...
 # Option 2: restarting DATASIM from a previous run
 echo "Updating DATASIM/DATASIM-UI old Public IP with new Public IP..." >> /home/ubuntu/datasimlog.txt
 export PUBLIC_DNS=`curl http://169.254.169.254/latest/meta-data/public-hostname 2>/dev/null`
 sudo sed -i 's|"http:\/\/[^:]*|"http://'$PUBLIC_DNS'|' /home/datasim/code/datasim-ui/dev.cljs.edn
 sudo sed -i 's|"http:\/\/[^:]*|"http://'$PUBLIC_DNS'|' /home/datasim/code/datasim/src/server/com/yetanalytics/datasim/server.clj
 echo "Creating DATASIM bundle from restart..." >> /home/ubuntu/datasimlog.txt
 cd /home/datasim/code/datasim
 make clean >> /home/ubuntu/datasimlog.txt
 make bundle >> /home/ubuntu/datasimlog.txt
 echo "Created DATASIM bundle following restart. Launching DATASIM API server..." >> /home/ubuntu/datasimlog.txt
 screen -dm -S "DATASIM" make server # opening a separate shell for the DATASIM API server
 sleep 60
 echo "DATASIM API server launched." >> /home/ubuntu/datasimlog.txt
 cd ../datasim-ui/
 ...
 echo "DATASIM-UI FIG launched. Launching DATASIM-UI SASS server..." >> /home/ubuntu/datasimlog.txt
 screen -dm -S "DATASIM-UI-SASS" make build-sass
 echo "DATASIM-UI SASS server launched. DATASIM successfully restarted." >> /home/ubuntu/datasimlog.txt
fi
